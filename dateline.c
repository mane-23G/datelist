...
Description: print out the date for the user
Purpose:
Usage: print date
Build with: gcc -o date date.c
Modifications:
*/
...
#define FORMAT  "%B %d %Y"              /* Default format string                  */
...
int parse_time_adjustment( char* datestring, struct tm* datetm, char format[])
{
    ...
    BOOL year = FALSE, month = FALSE, week = FALSE, day = FALSE, hour = FALSE, minute = FALSE;
...
int main(int argc, char *argv[]) {
    ...
    char schedule[MAXLEN], schedule_copy[MAXLEN];
    ...
    if(c_option) {
        ...
        if(errno == ERANGE && (count == LONG_MAX || count == LONG_MIN) || count <= 0)
    ...
    if(parse_time_adjustment(argv[optind], &time_adjustment, format) == -1) {
    ...
    if(argv[optind++] != NULL){
    	fprintf(stderr,"Extra argument found.\n");
    	sprintf(usage_msg,"%s [-c <count>] <schedule>",argv[0]);
	usage_error(usage_msg);
   }
